...
## 재개 방법 (2026-07-21 갱신)
1. 이 문서와 노션 페이지 확인
2. 완료: Loop 1~4, 6~10 + 연구 스파이크 기각·파기 + 코덱스 P0 정비 패키지 + v3 통합 재실행 + 층화 정확도 검증(코덱스 P1). 다음: **홀드아웃 검증(구 Loop 5) — 제3 영상 확보 시 최우선** (margin/floor/스무딩 파라미터 + 006 저합의 수용분 재점검 포함). 그 외 후보: Motion Profile Builder, 임포터 영속화(Blender add-on/TouchDesigner), 마커·halo 프레임별 페이드
