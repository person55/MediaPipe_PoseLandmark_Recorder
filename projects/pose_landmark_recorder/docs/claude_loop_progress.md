...
## 재개 방법 (2026-07-21 갱신)
1. 이 문서와 노션 페이지 확인
2. 완료: Loop 1~4, 6~10 + 연구 스파이크 기각·파기 + 코덱스 P0 정비 패키지 + v3 통합 재실행 + 층화 정확도 검증(코덱스 P1) + **홀드아웃 검증 1차(session_cpu_008, 통과)**. 다음 루프 후보: heel/foot 수용 가드(Loop 11), 교차 패스 합의 리포트 표준화(Loop 12), 마커·halo 프레임별 페이드. 잔여 외부 의존: 60fps 영상(fps 일반화 검증). 그 외: Motion Profile Builder, 임포터 영속화(Blender add-on/TouchDesigner)
